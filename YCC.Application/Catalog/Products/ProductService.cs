...
using YCC.Utilities.Exceptions;
...
namespace YCC.Application.Catalog.Products
{
    public class ProductService : IProductService
    {
        ...
        {
            var product = await _context.Products.FindAsync(productId);
            if (product == null) throw new YCCException($"Cannot find a product: {productId}");
            //lay ra danh sach anh cua san pham
        ...
        public async Task<ProductImageViewModel> GetImageById(int imageId)
        {
            ...
            if (image == null)
                throw new YCCException($"Cannot find an image with id {imageId}");
        ...
        public async Task<int> Update(ProductUpdateRequest request)
        {
            ...
            if (product == null || productTranslations == null) throw new YCCException($"Cannot find a product with id: {request.Id}");
        ...
        {
            var product = await _context.Products.FindAsync(productId);
            if (product == null) throw new YCCException($"Cannot find a product with id: {productId}");
        ...
        private async Task<string> SaveFile(IFormFile file)
        {
            ...
            //get cai Extension cua no -> tao ra 1 cai guid moi + voi ten file
            var fileName = $"{Guid.NewGuid()}{Path.GetExtension(originalFileName)}";
            //luu vao 1 cai storageService
